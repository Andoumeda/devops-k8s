...
echo "=== Pipeline 1: Compilacion (sin cache) ==="
...
echo "4. Build Docker Images (sin cache)..."
docker build --no-cache -t "$BACKEND_IMAGE:$IMAGE_TAG" "$SCRIPT_DIR/backend"
docker build --no-cache -t "$FRONTEND_IMAGE:$IMAGE_TAG" "$SCRIPT_DIR/frontend"
